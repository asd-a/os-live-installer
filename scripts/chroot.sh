...
usage: ${0##*/} chroot-dir [command] [arguments...]
    ...
    -L                  Run the local script but with chroot bind
...
if (( unshare )); then
  if (( exec_local )); then
    die 'local mode (-L) cannot be used in conjunction with unshare (-N)'
  fi
  if (( use_systemd )); then
  ...
  if (( exec_local )) && (( use_systemd )); then
    die 'systemd mode (-S) cannot be used in conjunction with local (-L)'
  fi
  setup=chroot_setup
  arch-chroot
fi